...
void Writer::createECChunks() {
  ...
  for (Symbol *s : target.symtab.expSymbols) {
    ...
    if (auto thunk = dyn_cast<ECExportThunkChunk>(sym->getChunk())) {
      ...
      Symbol *targetSym = target.symtab.find((targetName + "$hp_target").str());
      if (!targetSym)
        targetSym = target.symtab.find(targetName);
...
void Writer::setECSymbols() {
  ...
  Symbol *iatCopySym = target.symtab.findUnderscore("__hybrid_auxiliary_iat_copy");
  ...
  Symbol *delayIatSym =
      target.symtab.findUnderscore("__hybrid_auxiliary_delayload_iat");
  replaceSymbol<DefinedSynthetic>(
  ...
  Symbol *delayIatCopySym =
      target.symtab.findUnderscore("__hybrid_auxiliary_delayload_iat_copy");
  replaceSymbol<DefinedSynthetic>(
