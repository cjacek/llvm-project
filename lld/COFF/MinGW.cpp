...
AutoExporter::AutoExporter(
    COFFNamespace &ns, const llvm::DenseSet<StringRef> &manualExcludeSymbols)
    : manualExcludeSymbols(manualExcludeSymbols), ns(ns) {
  excludeLibs = {
      "libgcc",
      "libgcc_s",
      "libstdc++",
...
bool AutoExporter::shouldExport(Defined *sym) const {
  ...
  if (ns.symtab.find(("__imp_" + sym->getName()).str()))
    return false;
...
lld::coff::addWrappedSymbols(COFFNamespace &ns, opt::InputArgList &args) {
  ...
  for (auto *arg : args.filtered(OPT_wrap)) {
    ...
    Symbol *sym = ns.symtab.findUnderscore(name);
    if (!sym)
      continue;

    Symbol *real =
        ns.symtab.addUndefined(mangle("__real_" + name, ns.ctx.config.machine));
    Symbol *wrap =
        ns.symtab.addUndefined(mangle("__wrap_" + name, ns.ctx.config.machine));
...
void lld::coff::wrapSymbols(COFFNamespace &ns,
  ...
  for (const WrappedSymbol &w : wrapped) {
    ...
    if (Defined *d = dyn_cast<Defined>(w.wrap)) {
      Symbol *imp = ns.symtab.find(("__imp_" + w.sym->getName()).str());
      ...
      if (imp) {
        DefinedLocalImport *wrapimp = make<DefinedLocalImport>(
            ns.ctx, saver().save("__imp_" + w.wrap->getName()), d);
        ns.symtab.localImportChunks.push_back(wrapimp->getChunk());
        map[imp] = wrapimp;
      }
    }
  }
