...
bool LinkerDriver::findUnderscoreMangle(COFFNamespace &ns, StringRef sym) {
  Symbol *s = ns.symtab.findMangle(mangle(sym));
...
void LinkerDriver::parseDirectives(InputFile *file) {
  ...
  if (s.empty())
    return;
  COFFNamespace &ns = ctx.getNS(file->getMachineType());
  ...
  for (StringRef e : directives.exports) {
    ...
    // we dedup them first.
    if (!ns.directivesExports.insert(e).second)
      continue;
  ...
  for (auto *arg : directives.args) {
    ...
    case OPT_entry:
      ...
      ctx.forEachNS([&](COFFNamespace &ns) {
        ns.entry = addUndefined(ns, mangle(arg->getValue()), true);
      });
      break;
...
Symbol *LinkerDriver::addUndefined(COFFNamespace &ns, StringRef name,
                                   bool aliasEC) {
  Symbol *b = ns.symtab.addUndefined(name);
  if (!b->isGCRoot) {
    b->isGCRoot = true;
  ...
  if (aliasEC && ns.machine == ARM64EC) {
    if (std::optional<std::string> mangledName =
            getArm64ECMangledFunctionName(name)) {
      auto u = dyn_cast<Undefined>(b);
      if (u && !u->weakAlias) {
        Symbol *t = ns.symtab.addUndefined(saver().save(*mangledName));
        u->setWeakAlias(t, true);
      }
    } else if (std::optional<std::string> demangledName =
                   getArm64ECDemangledFunctionName(name)) {
      Symbol *us = ns.symtab.addUndefined(saver().save(*demangledName));
...
void LinkerDriver::addUndefinedGlob(StringRef arg) {
  ...
  ctx.forEachNS([&](COFFNamespace &ns) {
    ...
    ns.symtab.forEachSymbol([&syms, &pat](Symbol *sym) {
    ...
    for (Symbol *sym : syms)
      addUndefined(ns, sym->getName());
  });
}

StringRef LinkerDriver::mangleMaybe(COFFNamespace &ns, Symbol *s) {
  ...
  Symbol *mangled = ns.symtab.findMangle(unmangled->getName());
  if (!mangled)
    return "";
  ...
  unmangled->setWeakAlias(ns.symtab.addUndefined(mangled->getName()));
...
StringRef LinkerDriver::findDefaultEntry() {
  ...
  COFFNamespace &ns = ctx.primaryNS;
  if (ctx.config.subsystem == IMAGE_SUBSYSTEM_WINDOWS_GUI) {
    if (findUnderscoreMangle(ns, "wWinMain")) {
      if (!findUnderscoreMangle(ns, "WinMain"))
        return mangle("wWinMainCRTStartup");
      warn("found both wWinMain and WinMain; using latter");
    }
    return mangle("WinMainCRTStartup");
  }
  if (findUnderscoreMangle(ns, "wmain")) {
    if (!findUnderscoreMangle(ns, "main"))
...
WindowsSubsystem LinkerDriver::inferSubsystem() {
  ...
  // to not be called.
  COFFNamespace &ns = ctx.primaryNS;
  bool haveMain = findUnderscoreMangle(ns, "main");
  bool haveWMain = findUnderscoreMangle(ns, "wmain");
  bool haveWinMain = findUnderscoreMangle(ns, "WinMain");
  bool haveWWinMain = findUnderscoreMangle(ns, "wWinMain");
...
void LinkerDriver::createImportLibrary(bool asLib) {
  ...
  COFFNamespace &ns = ctx.primaryNS; // FIXME: ARM64X
  for (Export &e1 : ns.exports) {
    COFFShortExport e2;
...
void LinkerDriver::parseModuleDefs(COFFNamespace &ns, StringRef path) {
...
void LinkerDriver::maybeCreateECExportThunk(StringRef name, Symbol *&sym) {
  ...
  sym = addUndefined(ctx.getNS(ARM64EC), expName);
  if (auto undef = dyn_cast<Undefined>(sym)) {
...
void LinkerDriver::createECExportThunks() {
  ...
  COFFNamespace &ns = ctx.getNS(ARM64EC);
  for (Symbol *s : ns.symtab.expSymbols) {
    if (!s->isUsedInRegularObj)
      continue;
    assert(s->getName().starts_with("EXP+"));
    std::string targetName =
        (s->getName().substr(strlen("EXP+")) + "$hp_target").str();
    Symbol *sym = ns.symtab.find(targetName);
    if (!sym)
      continue;
    Defined *targetSym;
    if (auto undef = dyn_cast<Undefined>(sym))
  ...
  if (ns.entry)
    maybeCreateECExportThunk(ns.entry->getName(), ns.entry);
  for (Export &e : ns.exports) {
    if (!e.data)
...
void LinkerDriver::pullArm64ECIcallHelper() {
  ...
  if (!ctx.config.arm64ECIcallHelper)
    ctx.config.arm64ECIcallHelper = addUndefined(ns, "__icall_helper_arm64ec");
}
...
void LinkerDriver::maybeExportMinGWSymbols(COFFNamespace &ns,
  ...
  AutoExporter exporter(ns, excludedSymbols);
  ...
  ns.symtab.forEachSymbol([&](Symbol *s) {
...
void LinkerDriver::linkerMain(ArrayRef<const char *> argsArr) {
  ...
  COFFNamespace &ns = ctx.hybridNS ? *ctx.hybridNS : ctx.primaryNS;
  ...
  if (auto *arg = args.getLastArg(OPT_deffile)) {
    ...
    // FIXME: ARM64X
    parseModuleDefs(ns, arg->getValue());
  }

  // Handle generation of import library from a def file.
  if (!args.hasArg(OPT_INPUT, OPT_wholearchive_file)) {
    ctx.forEachNS([&](COFFNamespace &ns) { fixupExports(ns); });
    if (!config->noimplib)
  ...
  ctx.forEachNS([&](COFFNamespace &ns) {
    ...
    if (auto *arg = args.getLastArg(OPT_entry)) { // FIXME: ARM64X native entry?
      if (!arg->getValue()[0])
        fatal("missing entry point symbol name");
      ns.entry = addUndefined(ns, mangle(arg->getValue()), true);
    } else if (!ns.entry && !config->noEntry) {
      if (args.hasArg(OPT_dll)) {
        StringRef s = (config->machine == I386) ? "__DllMainCRTStartup@12"
                                                : "_DllMainCRTStartup";
        ns.entry = addUndefined(ns, s, true);
      } else if (config->driverWdm) {
        // /driver:wdm implies /entry:_NtProcessStartup
        ns.entry = addUndefined(ns, mangle("_NtProcessStartup"), true);
      } else {
  ...
  // Handle /delayload
  {
    ...
    for (auto *arg : args.filtered(OPT_delayload)) {
      ...
      ctx.forEachNS([&](COFFNamespace &ns) {
        if (ns.machine == I386) {
          ns.delayLoadHelper = addUndefined(ns, "___delayLoadHelper2@8");
        } else {
          ns.delayLoadHelper = addUndefined(ns, "__delayLoadHelper2", true);
        }
      });
    }
  }
  ...
  ctx.forEachNS([&](COFFNamespace &ns) {
    ns.symtab.addSynthetic(mangle("__ImageBase"), nullptr);
    if (ns.machine == I386) {
      ns.symtab.addAbsolute("___safe_se_handler_table", 0);
      ns.symtab.addAbsolute("___safe_se_handler_count", 0);
    }

    ns.symtab.addAbsolute(mangle("__guard_fids_count"), 0);
    ns.symtab.addAbsolute(mangle("__guard_fids_table"), 0);
    ns.symtab.addAbsolute(mangle("__guard_flags"), 0);
    ns.symtab.addAbsolute(mangle("__guard_iat_count"), 0);
    ns.symtab.addAbsolute(mangle("__guard_iat_table"), 0);
    ns.symtab.addAbsolute(mangle("__guard_longjmp_count"), 0);
    ns.symtab.addAbsolute(mangle("__guard_longjmp_table"), 0);
    // Needed for MSVC 2017 15.5 CRT.
    ns.symtab.addAbsolute(mangle("__enclave_config"), 0);
    // Needed for MSVC 2019 16.8 CRT.
    ns.symtab.addAbsolute(mangle("__guard_eh_cont_count"), 0);
    ns.symtab.addAbsolute(mangle("__guard_eh_cont_table"), 0);

    if (ns.machine == ARM64EC) {
      ns.symtab.addAbsolute("__arm64x_extra_rfe_table", 0);
      ns.symtab.addAbsolute("__arm64x_extra_rfe_table_size", 0);
      ns.symtab.addAbsolute("__arm64x_native_entrypoint", 0);
      ns.symtab.addAbsolute("__arm64x_redirection_metadata", 0);
      ns.symtab.addAbsolute("__arm64x_redirection_metadata_count", 0);
      ns.symtab.addAbsolute("__hybrid_auxiliary_delayload_iat_copy", 0);
      ns.symtab.addAbsolute("__hybrid_auxiliary_delayload_iat", 0);
      ns.symtab.addAbsolute("__hybrid_auxiliary_iat", 0);
      ns.symtab.addAbsolute("__hybrid_auxiliary_iat_copy", 0);
      ns.symtab.addAbsolute("__hybrid_code_map", 0);
      ns.symtab.addAbsolute("__hybrid_code_map_count", 0);
      ns.symtab.addAbsolute("__hybrid_image_info_bitfield", 0);
      ns.symtab.addAbsolute("__x64_code_ranges_to_entry_points", 0);
      ns.symtab.addAbsolute("__x64_code_ranges_to_entry_points_count", 0);
      ns.symtab.addSynthetic("__guard_check_icall_a64n_fptr", nullptr);
    }

    if (config->pseudoRelocs) {
      ns.symtab.addAbsolute(mangle("__RUNTIME_PSEUDO_RELOC_LIST__"), 0);
      ns.symtab.addAbsolute(mangle("__RUNTIME_PSEUDO_RELOC_LIST_END__"), 0);
    }
    if (config->mingw) {
      ns.symtab.addAbsolute(mangle("__CTOR_LIST__"), 0);
      ns.symtab.addAbsolute(mangle("__DTOR_LIST__"), 0);
    }
    if (config->debug || config->buildIDHash != BuildIDHash::None)
      if (ns.symtab.findUnderscore("__buildid"))
        ns.symtab.addUndefined(mangle("__buildid"));
  });
  ...
  // converge.
  {
    ...
    do {
      ctx.forEachNS([&](COFFNamespace &ns) {
        ...
        if (ns.entry)
          mangleMaybe(ns, ns.entry);

        // Windows specific -- Make sure we resolve all dllexported symbols.
        for (Export &e : ns.exports) {
          if (!e.forwardTo.empty())
            continue;
          e.sym = addUndefined(ns, e.name, !e.data);
          if (e.source != ExportSource::Directives)
            e.symbolName = mangleMaybe(ns, e.sym);
        }
        ...
        for (auto pair : config->alternateNames) {
          ...
          Symbol *sym = ns.symtab.find(from);
          if (!sym)
            continue;
          if (auto *u = dyn_cast<Undefined>(sym)) {
            ...
            u->setWeakAlias(ns.symtab.addUndefined(to));
          }
        }
        ...
        if (!ctx.bitcodeFileInstances.empty()) {
          ...
          for (auto *s : lto::LTO::getRuntimeLibcallSymbols(TT))
            ns.symtab.addLibcall(s);
        }

        // Windows specific -- if __load_config_used can be resolved, resolve
        // it.
        if (ns.symtab.findUnderscore("_load_config_used"))
          addUndefined(ns, mangle("_load_config_used"));

        if (args.hasArg(OPT_include_optional)) {
          // Handle /includeoptional
          for (auto *arg : args.filtered(OPT_include_optional))
            if (isa_and_nonnull<LazyArchive>(ns.symtab.find(arg->getValue())))
              addUndefined(ns, arg->getValue());
        }
      });
    } while (run());
  ...
  std::vector<WrappedSymbol> wrapped = addWrappedSymbols(ns, args);
  ...
  if (config->autoImport || config->stdcallFixup) {
    ...
    ctx.forEachNS([](COFFNamespace &ns) { ns.symtab.loadMinGWSymbols(); });
    run();
  }
  ...
  if (!ctx.bitcodeFileInstances.empty() && !config->forceUnresolved)
    ctx.forEachNS([](COFFNamespace &ns) { ns.symtab.reportUnresolvable(); });
  if (errorCount())
    return;

  ctx.forEachNS([&](COFFNamespace &ns) {
    ns.hadExplicitExports = !ns.exports.empty();
    if (config->mingw) {
      // In MinGW, all symbols are automatically exported if no symbols
      // are chosen to be exported.
      maybeExportMinGWSymbols(ns, args);
    }
  });
  ...
  ctx.forEachNS([](COFFNamespace &ns) { ns.symtab.compileBitcodeFiles(); });

  ctx.forEachNS([&](COFFNamespace &ns) {
    if (Defined *d =
            dyn_cast_or_null<Defined>(ns.symtab.findUnderscore("_tls_used")))
  ...
  if (!wrapped.empty()) {
    ctx.forEachNS([&](COFFNamespace &ns) { wrapSymbols(ns, wrapped); });
  }
  ...
  ctx.forEachNS([&](COFFNamespace &ns) {
    while (ns.symtab.resolveRemainingUndefines())
      run();
  });
  ...
  ctx.forEachNS([&](COFFNamespace &ns) {
    if (!ns.exports.empty() || config->dll) {
  ...
  ctx.forEachNS([&](COFFNamespace &ns) {
    if (!ns.exports.empty() || config->dll)
      assignExportOrdinals(ns);
  });

  // Handle /output-def (MinGW specific).
  // FIXME: ARM64X
  if (auto *arg = args.getLastArg(OPT_output_def))
    writeDefFile(arg->getValue(), ns.exports);
  ...
  ctx.forEachNS([&](COFFNamespace &ns) {
    ...
    for (auto pair : config->alignComm) {
      ...
      Symbol *sym = ns.symtab.find(name);
      if (!sym) {
    ...
    if (config->mingw) {
      ...
      ctx.forEachNS([&](COFFNamespace &ns) {
        for (const char *n : {"__gxx_personality_v0", "__gcc_personality_v0",
                              "rust_eh_personality"}) {
          Defined *d = dyn_cast_or_null<Defined>(ns.symtab.findUnderscore(n));
          if (d && !d->isGCRoot) {
            d->isGCRoot = true;
            config->gcroot.push_back(d);
          }
