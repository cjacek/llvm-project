...
void LinkerDriver::parseDirectives(InputFile *file) {
  ...
  for (StringRef e : directives.excludes) {
    ...
    for (StringRef sym : vec)
      excludedSymbols.insert(ctx.symtab.mangle(sym));
  }
...
WindowsSubsystem LinkerDriver::inferSubsystem() {
  ...
  // to not be called.
  bool haveMain = ctx.symtab.findUnderscoreMangle("main");
  bool haveWMain = ctx.symtab.findUnderscoreMangle("wmain");
  bool haveWinMain = ctx.symtab.findUnderscoreMangle("WinMain");
  bool haveWWinMain = ctx.symtab.findUnderscoreMangle("wWinMain");
...
void LinkerDriver::linkerMain(ArrayRef<const char *> argsArr) {
  ...
  for (auto *arg : args.filtered(OPT_incl)) {
    if (ctx.hybridSymtab)
      ctx.hybridSymtab->addGCRoot(arg->getValue());
    else
      ...
      ctx.forEachSymtab([&](SymbolTable &symtab) {
        if (symtab.machine == I386) {
          symtab.delayLoadHelper =
              symtab.addGCRoot("___delayLoadHelper2@8");
        } else {
          symtab.delayLoadHelper =
              symtab.addGCRoot("__delayLoadHelper2", true);
        }
      });
    }
  }
