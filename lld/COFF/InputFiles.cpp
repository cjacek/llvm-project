...
Symbol *ImportFile::findECExitThunkSymbol() const {
  ...
  if (!chkECSym)
    return nullptr;
  if (Symbol *sym = target.symtab.findECThunk(impECSym, Arm64ECThunkType::Exit))
    return sym;
  if (Symbol *sym = target.symtab.findECThunk(thunkSym, Arm64ECThunkType::Exit))
    return sym;
  if (Symbol *sym = target.symtab.findECThunk(impSym, Arm64ECThunkType::Exit))
    return sym;
  return nullptr;
}
