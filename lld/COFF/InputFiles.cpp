  ...
  if (isArm64EC(ctx.config.machine)) {
	...
	if (isArm64ECMangledFunctionName(sym.getName())) {
	    StringRef unmangledName = saver().save(getArm64ECDemangledFunctionName(sym.getName()));
            target.symtab.addLazyArchive(this, sym, unmangledName);
        }
      }
    ...
    for (const Archive::Symbol &sym : file->symbols()) {
      if (isArm64ECMangledFunctionName(sym.getName())) {
        StringRef unmangledName =
            saver().save(getArm64ECDemangledFunctionName(sym.getName()));
        ctx.primaryTarget.symtab.addLazyArchive(this, sym, unmangledName);
      }
    }
  }
}
  ...
  for (SectionChunk *chunk : ECThunksMapChunks) {
    ...
    while (entry < end) {
      if (entry->type != 0 && entry->type != 1 && entry->type != 4)
        ...
        ctx.getTarget(ARM64EC).symtab.addECThunk(
            getSymbol(entry->src), getSymbol(entry->dst), entry->type);
      entry++;
    }
  }
}
...
void ImportFile::parse() {
  ...
  if (isCode) {
    if (target.machine != ARM64EC) {
      ...
      auxThunkSym =
          target.symtab.addImportThunk(auxThunkName, impECSym, ARM64EC);
...
Symbol *ImportFile::findECExitThunkSymbol() const {
  ...
  if (!chkECSym)
    return nullptr;
  if (Symbol *sym = target.symtab.findECThunk(impECSym, 4))
    return sym;
  if (Symbol *sym = target.symtab.findECThunk(thunkSym, 4))
    return sym;
  if (Symbol *sym = target.symtab.findECThunk(impSym, 4))
    return sym;
  return nullptr;
}
