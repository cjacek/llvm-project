...
class COFFNamespace {
public:
  COFFNamespace(COFFLinkerContext &ctx) : ctx(ctx), symtab(*this) {}
...
public:
  ...
  COFFNamespace primaryNS;
  std::optional<COFFNamespace> hybridNS;

  void forEachNS(std::function<void(COFFNamespace &ctx)> f) {
    f(primaryNS);
    if (hybridNS)
      f(*hybridNS);
  }

  COFFNamespace &getNS(llvm::COFF::MachineTypes machine) {
    if (hybridNS && (machine == ARM64EC || machine == AMD64))
      return *hybridNS;
    return primaryNS;
  }
};
