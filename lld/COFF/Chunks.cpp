...
uint32_t ImportThunkChunkARM64EC::verifyRanges(bool fixup) {
  if (extend)
    return false;
  auto helper = dyn_cast<Defined>(file->ctx.config.arm64ECIcallHelper);
  if (!helper)
    return 0;
  int64_t diff = helper->getRVA() - rva - 16;
  if (isInt<28>(diff))
    return 0;
  extend = fixup;
