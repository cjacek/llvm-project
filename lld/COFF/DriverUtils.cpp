...
void LinkerDriver::fixupExports(COFFNamespace &ns) {
  ...
  // Uniquefy by name.
  DenseMap<StringRef, std::pair<Export *, unsigned>> map(ns.exports.size());
  std::vector<Export> v;
  for (Export &e : ns.exports) {
  ...
  ns.exports = std::move(v);

  // Sort by name.
  llvm::sort(ns.exports, [](const Export &a, const Export &b) {
    return a.exportName < b.exportName;
  });
}

void LinkerDriver::assignExportOrdinals(COFFNamespace &ns) {
