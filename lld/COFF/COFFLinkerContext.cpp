...
COFFLinkerContext::COFFLinkerContext()
    : driver(*this), ltoTextSection(llvm::COFF::IMAGE_SCN_MEM_EXECUTE),
      ...
      diskCommitTimer("Commit to Disk", totalPdbLinkTimer), primaryNS(*this) {}
...
void COFFLinkerContext::setMachine(MachineTypes machine) {
  ...
  if (machine != ARM64X) {
    primaryNS.machine = machine;
  } else {
    primaryNS.machine = ARM64;
    hybridNS.emplace(*this);
    hybridNS->machine = ARM64EC;
  }
