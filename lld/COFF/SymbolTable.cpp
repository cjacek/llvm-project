...
void SymbolTable::loadMinGWSymbols() {
  for (auto &i : symMap) {
    ...
    if (ns.machine == I386 && ns.ctx.config.stdcallFixup) {
      ...
      if (newName != origName && (l = find(newName)) != nullptr) {
        ...
        if (l->isLazy() || isa<Defined>(l)) {
          if (ns.ctx.config.warnStdcallFixup)
...
bool SymbolTable::handleMinGWAutomaticImport(Symbol *sym, StringRef name) {
  ...
  if (refptr && refptr->getChunk()->getSize() == ns.ctx.config.wordsize) {
...
void SymbolTable::reportUnresolvable() {
  ...
  for (auto &i : symMap) {
    ...
    if (name.contains("_PchSym_"))
      continue;
    if (ns.ctx.config.autoImport && impSymbol(name))
      continue;
    undefs.insert(sym);
  }

  reportProblemSymbols(ns.ctx, undefs,
  ...
  for (auto &i : symMap) {
    ...
    if (name.starts_with("__imp_")) {
      ...
      if (imp && isa<Defined>(imp)) {
        ...
        replaceSymbol<DefinedLocalImport>(sym, ns.ctx, name, d);
    ...
    if (ns.ctx.config.autoImport && handleMinGWAutomaticImport(sym, name))
      continue;

    // Remaining undefined symbols are not fatal if /force is specified.
    // They are replaced with dummy defined symbols.
    if (ns.ctx.config.forceUnresolved)
      replaceSymbol<DefinedAbsolute>(sym, ns.ctx, name, 0);
    undefs.insert(sym);
  }

  reportProblemSymbols(ns.ctx, undefs,
                       ns.ctx.config.warnLocallyDefinedImported ? &localImports
                                                                : nullptr,
                       false);
  return foundLazy;
}
...
std::pair<Symbol *, bool> SymbolTable::insert(StringRef name) {
  ...
  if (!sym) {
    ...
    if (isArm64EC(ns.machine) && name.starts_with("EXP+"))
...
void SymbolTable::initializeECThunks() {
  if (!isArm64EC(ns.ctx.config.machine))
    return;
  ...
  for (ImportFile *file : ns.ctx.importFileInstances) {
    if (!file->impchkThunk)
      continue;
  ...
  parallelForEach(ns.ctx.objFileInstances, [&](ObjFile *file) {
...
void SymbolTable::addLazyArchive(ArchiveFile *f, const Archive::Symbol &sym) {
  ...
  if (isArm64EC(ns.machine) && !checkLazyECPair<LazyArchive>(this, name, f))
    return;
  ...
  if (!u || (u->weakAlias && !u->isECAlias(ns.machine)) ||
...
void SymbolTable::addLazyObject(InputFile *f, StringRef n) {
  assert(f->lazy);
  if (isArm64EC(ns.machine) && !checkLazyECPair<LazyObject>(this, n, f))
    return;
...
void SymbolTable::reportDuplicate(Symbol *existing, InputFile *newFile,
  ...
  os << "duplicate symbol: " << toString(ns.ctx, *existing);
  ...
  if (ns.ctx.config.forceMultiple)
    warn(msg);
  else
    error(msg);
}
...
Symbol *SymbolTable::addAbsolute(StringRef n, COFFSymbolRef sym) {
  ...
  if (wasInserted || isa<Undefined>(s) || s->isLazy())
    replaceSymbol<DefinedAbsolute>(s, ns.ctx, n, sym);
...
Symbol *SymbolTable::addAbsolute(StringRef n, uint64_t va) {
  ...
  s->isUsedInRegularObj = true;
  if (wasInserted || isa<Undefined>(s) || s->isLazy())
    replaceSymbol<DefinedAbsolute>(s, ns.ctx, n, va);
  else if (auto *da = dyn_cast<DefinedAbsolute>(s)) {
    if (da->getVA() != va)
      reportDuplicate(s, nullptr);
  } else if (!isa<DefinedCOFF>(s))
...
Defined *SymbolTable::addImportThunk(StringRef name, DefinedImportData *id,
  ...
  s->isUsedInRegularObj = true;
  if (wasInserted || isa<Undefined>(s) || s->isLazy()) {
    replaceSymbol<DefinedImportThunk>(s, ns.ctx, name, id, chunk);
...
void SymbolTable::compileBitcodeFiles() {
  if (ns.ctx.bitcodeFileInstances.empty())
    return;

  llvm::TimeTraceScope timeScope("Compile bitcode");
  ScopedTimer t(ns.ctx.ltoTimer);
  lto.reset(new BitcodeCompiler(ns.ctx));
  for (BitcodeFile *f : ns.ctx.bitcodeFileInstances)
    lto->add(*f);
  for (InputFile *newObj : lto->compile()) {
    ObjFile *obj = cast<ObjFile>(newObj);
    obj->parse();
    ns.ctx.objFileInstances.push_back(obj);
  }
}
