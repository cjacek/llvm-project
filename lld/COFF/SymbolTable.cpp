...
bool SymbolTable::resolveRemainingUndefines() {
  ...
  reportProblemSymbols(
      target.ctx, undefs,
      target.ctx.config.warnLocallyDefinedImported ? &localImports : nullptr, false);
  return foundLazy;
}
...
void SymbolTable::initializeECThunks() {
  if (!isArm64EC(target.ctx.config.machine))
    return;
...
void SymbolTable::addLazyArchive(ArchiveFile *f, const Archive::Symbol &sym) {
  ...
  if (!u || (u->weakAlias && !u->isECAlias(target.machine)) ||
...
void SymbolTable::addLazyObject(InputFile *f, StringRef n) {
  assert(f->lazy);
  if (isArm64EC(target.machine) && !checkLazyECPair<LazyObject>(this, n, f))
    return;
